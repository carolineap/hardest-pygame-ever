    ...
    while(j < max_iterations and winners < min_winners):
        ...
        for i in range(int(len(parents_selection)/2)):
            ...
            if random.random() <= mutation_rate:
                new_ind_1_crom = ga.mutation(new_ind_1_crom, len(new_ind_1_crom), state_size - change_point, parents_selection[2*i].death)
                new_ind_2_crom = ga.mutation(new_ind_2_crom, len(new_ind_2_crom), state_size - change_point, parents_selection[(2*i)+1].death)
